...
public class GoogleStaticMap : MonoBehaviour {
	...
	public void initialize () {
		...
		float zoomDiff = 17 - zoom;

		//Debug.Log ("XOOM HERE "+zoom);
		float scale = 0.1f/zoomDiff;
		//realWorldtoUnityWorldScale = new Vector2 (scale, scale); 
		realWorldtoUnityWorldScale = new Vector2 (0.1f, 0.1f); 
	}

	public void copyFrom (GoogleStaticMap old) {
		
		horizontalSize = old.horizontalSize;

		verticalSize = old.verticalSize;

		//initialPixelToMercator = 2.0f / ((float) TILE_SIZE); 

		curPixelToMercator = old.curPixelToMercator;

		widthMercatorX = old.widthMercatorX;
		heightMercatorY = old.heightMercatorY;

		// You can initialize realWorldtoUnityWorldScale to another value of your choice.
		realWorldtoUnityWorldScale = old.realWorldtoUnityWorldScale; 
	}
	...
	IEnumerator DrawMapCoroutine ()
	{
		if (MAX_MAP_REQUEST_NUM != 0 && countOfMapRequests < MAX_MAP_REQUEST_NUM) {
			...
			isDrawn = true;
			Debug.Log ("NUM REQUESTS: " + countOfMapRequests);
		} else {
	...
	public void findCorners(){
		...
		Debug.Log (SW_Mercator);
		Debug.Log("AFTER DRAWING: "+mercatorPointToGeoPoint (SW_Mercator));
